...
// Zoho Cliq Webhook
const BOT_WEBHOOK_URL = 'https://cliq.zoho.com/api/v2/bots/afterworkbuddy/incoming';
const BOT_AUTH_TOKEN = process.env.BOT_AUTH_TOKEN;

// Send message to user via Zoho Cliq webhook
async function sendCliqMessage(userId, userName, message) {
    try {
        ...
        console.error('Error sending message via webhook', err.message);
    }
}
...
async function processUserChannels(userId) {
    try {
        ...
        if (!channels || !workStart || !workEnd) return null;
        ...
        if (manualOverride !== undefined && overrideUntil) {
            ...
            if (now < until) shouldMute = manualOverride;
        }

        const currentStatus = shouldMute ? 'Work Time (Please mute channels)' : 'Relax Time (Please unmute channels)';
...
app.post('/incoming', async (req, res) => {
    try {
        ...
        if (!userId) return res.status(400).json({ text: "❌ User ID not found" });
...
app.listen(PORT, () => {
    ...
    console.log('Incoming Webhook Endpoint: ' + BOT_WEBHOOK_URL);
});
